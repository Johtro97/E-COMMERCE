...
export const OpenCart = () =>{
  ...
  const checkout = () =>{
    ...
    return (
            <section className="flex flex-col text-sm space-y-9 mt-5 p-4">
              {cart.map((product, i) =>(
               <div key={i}>
                 <button onClick={()=> removeFromCart(product)} className=" absolute right-10 hover:underline hover:text-red-400"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                ...
                <div><img src={product.image} width={50} height={50}/></div>
                  ...
                  <section className="flex space-x-5 ">
                    ...
                    <p>$ {parseFloat(product.price * product.amount).toFixed(2)}</p>
            ...
            <div>{user === null ? 
              ...
              : <div>
              {cart.length === 0 ? 
                <Link to={"/products"} onClick={handleClick} className="w-36 bg-orange-50 shadow-md p-2 rounded-md hover:underline hover:text-red-400">Go to products</Link> 
                :<Link to={"/"} onClick={() => checkout()}className="w-36 bg-orange-50 shadow-md p-2 rounded-md hover:underline hover:text-red-400">Checkout</Link>
              }
            </div>
          }</div>
          </section>
